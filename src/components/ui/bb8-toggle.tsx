// Ported from a pasted "BB-8 droid" theme-toggle (styled-components) — this
// project has no styled-components dependency, so the CSS body (already
// self-contained under its own `.bb8-toggle` root class, no JS
// interpolation inside it) is dropped into a plain <style> tag instead of a
// styled.div, and the checkbox becomes a real controlled input wired to
// this site's own isLight/onThemeChange state rather than local UI state.
// Unchecked = day sky (light theme), checked = night sky (dark theme) —
// that's the original design's own mapping (background-position-y shifts
// from the gradient's lower/blue half to its upper/night half on :checked),
// not something added here. The actual theme flip runs through the same
// circular-wipe View Transition as the Control Center's own toggle (see
// lib/theme-transition.ts) rather than switching instantly, so both
// controls feel like the same control.
...
export function BB8Toggle({ theme, onThemeChange, className }: BB8ToggleProps) {
  ...
  .bb8:hover .bb8__head::after { background-position: 0 0 !important; }
      `}</style>
      <label ref={labelRef} className="bb8-toggle">
        <input
          ...
          onChange={(e) => {
            ...
            if (!el) { onThemeChange(next); return; }
            runThemeTransition(el, 500, () => onThemeChange(next));
          }}
