// MagicUI's Smooth Cursor (magicui.design/r/smooth-cursor.json), ported onto
// this project's `framer-motion` import (same package as `motion/react` —
// "Motion" is the library's current name, `framer-motion` its compat
// package — kept for consistency with every other component here).
...
function DefaultCursorSVG() {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" width={50} height={54} viewBox="0 0 50 54" fill="none" style={{ scale: 0.5 }}>
      ...
      <defs>
        <filter id="smooth-cursor-shadow" x={0.6} y={0.95} width={49.06} height={52.43} filterUnits="userSpaceOnUse" colorInterpolationFilters="sRGB">
...
export function SmoothCursor() {
  ...
  // Only takes over the cursor on devices that actually have a fine
  // mouse-like pointer — never on touch, where there's no hover cursor to
  ...
  return (
    <motion.div
      style={{
        ...
        rotate: rotation,
        scale,
        // Above everything else in the app, including full-screen AppWindows
        // (z-[100000]) — a cursor that could hide behind foreground UI isn't
        // a cursor.
        zIndex: 999999,
        pointerEvents: 'none',
