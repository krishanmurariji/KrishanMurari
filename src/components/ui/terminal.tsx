// Ported from MagicUI's Terminal component (magicui.design/r/terminal.json):
// Terminal (the window chrome + sequencing context), AnimatedSpan (a line
// that fades/slides in, in sequence), and TypingAnimation (a line that types
// itself out character by character). Kept faithful to the source — the
// sequence-context bookkeeping (each child waits for the previous one to
// finish before starting) is real, non-obvious logic, not something to trim.
// Two changes from the registry version: `framer-motion` import (this
// project's convention — same library as `motion/react`, just the compat
// package name everything else here already uses) and `cn` from the
// project's own relative `lib/utils` instead of the `@/` alias, which
...
export const Terminal = ({ children, className, sequence = true, startOnView = true }: TerminalProps) => {
  ...
  const content = (
    <div
      ref={containerRef}
      className={cn('bg-[#161616] z-0 h-full max-h-100 w-full max-w-lg rounded-xl border border-white/10', className)}
    >
      <div className="border-white/10 flex flex-col gap-y-2 border-b p-4">
      ...
      <pre className="p-4">
        <code className="grid gap-y-1 overflow-auto text-white/90">{wrappedChildren}</code>
      </pre>
    </div>
  );
